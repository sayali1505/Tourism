...
<?php 
  ...
  <img class=img src="data:image/jpeg;base64,'.base64_encode($row['image']).'" alt=John style=width:100% ;height:20%><br>
  <h3>'.($row['pac_name']).'</h3><br>
  <h4>'.$package.'</h4><br>
    Days:'.($row['days']).'<br>
    <a href=view-details.php><p><input type=button name='.$row['pac_name'].'>View Details</button></p></a>
</div></div></body>
</html>';
